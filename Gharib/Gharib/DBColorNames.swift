...
//  ColorDetection
//
//  Created by ZEUS on 04/03/2019.
//  Copyright © 2019 Yuri Y. All rights reserved.
//

import Foundation
import UIKit

var kID:NSString! = "colorId"
...
func colorFromRGBFloat(rgbValue:Int) -> UIColor {
    let redColor = Double((rgbValue & 0xFF0000) >> 16)
    let greenColor = Double((rgbValue & 0xFF00) >> 8)
    let blueColor = Double((rgbValue & 0xFF))
    let returnColor = UIColor.init(red: CGFloat(redColor/255.0), green: CGFloat(greenColor/255.0), blue: CGFloat(blueColor/255.0), alpha: 1.0)
    return returnColor
//    return UIColor.init(red: ((rgbValue & 0xFF0000) >> 16)) / 255.0 green: ((rgbValue & 0xFF00) >> 8)) / 255.0, blue: (rgbValue & 0xFF)) / 255.0, alpha: 1.0)
}

public func nameForColor(colorA:UIColor)->String {
    var iR:CGFloat = 0.0
    var iG:CGFloat = 0.0
    var iB:CGFloat = 0.0
    var iA:CGFloat = 0.0
    
    colorA.getRed(&iR, green: &iG, blue: &iB, alpha: &iA)
    ...
    for i in 0 ..< colorMap.count {
        let tempArray = colorMap[i] as! NSArray
        let tempHex = tempArray.object(at: 1) as! Int

        var currentColor:UIColor = colorFromRGBFloat(rgbValue: tempHex)
        ...
        var dR:Double = Double(iR - cR)
        var dG:Double = Double(iG - cG)
        var dB:Double = Double(iB - cB)

        var a:Double = pow(dR, 2.0)
        var b:Double = pow(dG, 2.0)
        var c:Double = pow(dB, 2.0)

        var d:Double = sqrt(a+b+c)

        let dFloat = CGFloat(d)
        let dMindistance = Double(minDistance)
        
        if (d < dMindistance) {
            minDistance = CGFloat(d)
            colorKey = tempArray.object(at: 0) as! String
        }
    }
